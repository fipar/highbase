...
read -s password
echo "Please enter the datadir for the slave node (i.e. /var/lib/mysql)">&2
read slavedatadir
...
ssh root@$slave "echo SLAVE STOP|mysql -uroot -p$password; echo LOAD DATA FROM MASTER|mysql -uroot -p$password; echo RESET SLAVE|mysql -uroot -p$password; echo SLAVE START|mysql -uroot -p$password; cd $slavedatadir; tar xjvf /tmp/mysql-snapshot.tar.bz2; rm -f /tmp/mysql-snapshot.tar.bz2"
...
DATADIR=$(get_datadir)
cd $DATADIR
echo "once again, please enter mysql's root password for the master node">&2
cat <<EOSCR | mysql -uroot -p
FLUSH TABLES WITH READ LOCK;
\! tar cjvf /tmp/mysql-snapshot.tar.bz2 . --exclude=mysql
UNLOCK TABLES;
EOSCR
